...
def generate_config_collect_class():
    """在 generated_classes.hpp 中生成线程安全的懒汉式单例 ConfigCollect 类，包含所有顶级类的私有成员变量、公共 getter 方法以及 update_from_yaml 方法。
    ...
    with open("generated_classes.hpp", "a") as f:
        ...
        for full_class_name in generated_top_classes:
            ...
            f.write("    }\n\n")
        f.write("};\n\n")
    print("已生成线程安全的懒汉式单例 ConfigCollect 类，增加了 update_from_yaml_all 函数，确保调用时的字符串中不会出现连续的下划线。")
