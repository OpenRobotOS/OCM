...
def generate_class(class_name, data, namespaces, output_path):
    ...
    for key, value in data.items():
        if isinstance(value, dict):
            ...
            # **Update:** Append 'auto_' prefixed namespace based on the key for nested classes
            new_namespaces = namespaces + [f"auto_{capitalize(key)}"]
            generate_class(sub_class_name, value, new_namespaces, output_path)
        elif isinstance(value, list) and len(value) > 0 and isinstance(value[0], dict):
            # 假设列表名为复数形式，类名为单数
            sub_class_name = capitalize(key.rstrip('s'))
            # **Update:** Append 'auto_' prefixed namespace based on the key for nested classes
            new_namespaces = namespaces + [f"auto_{capitalize(key.rstrip('s'))}"]
    ...
    for root, dirs, files in os.walk(input_path):
        for file in files:
            if file.endswith(('.yaml', '.yml')):
                ...
                # 类名与文件名相同
                class_name = file_namespace+"_main"
                # 读取YAML文件
                try:
