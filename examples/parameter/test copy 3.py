...
# 用于存储所有生成的类，用于生成 ConfigCollect 类
...
def generate_class(class_name, data, namespaces):
    ...
    # 记录所有生成的类（全限定名）
    all_generated_classes.append(full_class_name)

def generate_config_collect():
    """生成 ConfigCollect 类代码并写入文件"""
    class_code = []
    class_code.append("class ConfigCollect {")
    class_code.append("public:")

    # 生成成员变量，每个 YAML 文件对应一个类
    for full_class_name in all_generated_classes:
        # 提取类名（最后一个命名空间部分）
        class_name = full_class_name.split("::")[-1]
        member_var_name = class_name.lower()  # 使用小写类名作为成员变量名
        class_code.append(f"    {class_name} {member_var_name};")

    class_code.append("")
    # 生成 update_from_yaml 方法
    class_code.append("    void update_from_yaml(const YAML::Node& node) {")

    for full_class_name in all_generated_classes:
        class_name = full_class_name.split("::")[-1]
        member_var_name = class_name.lower()
        class_code.append(f"        if(node[\"{class_name}\"]) {member_var_name}.update_from_yaml(node[\"{class_name}\"]);")

    class_code.append("    }")
    class_code.append("};")
    class_code.append("")  # 空行

    # 将 ConfigCollect 类写入文件
    with open("generated_classes.hpp", "a") as f:
        for line in class_code:
            f.write(line + "\n")
