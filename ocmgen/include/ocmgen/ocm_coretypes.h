...
typedef struct ___ocm_hash_ptr __ocm_hash_ptr;
struct ___ocm_hash_ptr {
  const __ocm_hash_ptr *parent;
  int64_t (*v)(void);
};
...
static inline int __string_encoded_size(char *const *s) { return sizeof(int64_t) + __string_encoded_array_size(s, 1); }
...
static inline void *ocm_malloc(size_t sz) {
  if (sz) return malloc(sz);
  return NULL;
}

/**
 * Describes the type of a single field in an OCM message.
 */
typedef enum {
  OCM_FIELD_INT8_T,
  OCM_FIELD_INT16_T,
...
#define OCM_TYPE_FIELD_MAX_DIM 50

/**
 * Describes a single ocmtype field's datatype and array dimmensions
 */
typedef struct _ocm_field_t ocm_field_t;
struct _ocm_field_t {
  /**
   * name of the field
   */
   ...
   **/
  ocm_field_type_t type;

  /**
   * datatype of the field (in string format)
   * this should be the same as in the ocm type decription file
   */
  const char *typestr;
   ...
   */
  int32_t dim_size[OCM_TYPE_FIELD_MAX_DIM];

  /**
   * a boolean describing whether the dimension is
   * variable. Valid on [0:num_dim-1].
   */
  int8_t dim_is_variable[OCM_TYPE_FIELD_MAX_DIM];
...
typedef int (*ocm_encode_t)(void *buf, int offset, int maxlen, const void *p);
typedef int (*ocm_decode_t)(const void *buf, int offset, int maxlen, void *p);
typedef int (*ocm_decode_cleanup_t)(void *p);
typedef int (*ocm_encoded_size_t)(const void *p);
typedef int (*ocm_struct_size_t)(void);
typedef int (*ocm_num_fields_t)(void);
typedef int (*ocm_get_field_t)(const void *p, int i, ocm_field_t *f);
typedef int64_t (*ocm_get_hash_t)(void);

/**
 * Describes an ocmtype info, enabling introspection
 */
typedef struct _ocm_type_info_t ocm_type_info_t;
struct _ocm_type_info_t {
  ocm_encode_t encode;
  ocm_decode_t decode;
  ocm_decode_cleanup_t decode_cleanup;
  ocm_encoded_size_t encoded_size;
  ocm_struct_size_t struct_size;
  ocm_num_fields_t num_fields;
  ocm_get_field_t get_field;
  ocm_get_hash_t get_hash;
};
