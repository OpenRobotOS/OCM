...
#else
  int len = strlen(path);
  char *dirpath = static_cast<char *>(std::malloc(len + 1));
  for (int i = 0; i < len; i++) {
    if (path[i] == '/') {
...
static char *build_filenamev(char **parts) {
  ...
  for (p = parts; *p; p++) {
    ...
    if (*(p + 1)) strncat(result, G_DIR_SEPARATOR_S, total_len);
  }
  return result;
}
...
static int is_same_type(const ocm_typename_t *tn1, const ocm_typename_t *tn2) { return !strcmp(tn1->lctypename, tn2->lctypename); }

// static int
// is_same_package (const ocm_typename_t *tn1, const ocm_typename_t *tn2) {
//    return ! strcmp (tn1->package, tn2->package);
//}

static char _struct_format(ocm_member_t *lm) {
...
static void _emit_decode_one(const ocmgen_t *ocm, FILE *f, ocm_struct_t *ls, ocm_member_t *lm, const char *accessor, int indent, const char *sfx) {
  const char *tn = lm->type->lctypename;
...
static void _emit_decode_list(const ocmgen_t *ocm, FILE *f, ocm_struct_t *ls, ocm_member_t *lm, const char *accessor, int indent, int is_first,
  ...
  if (!strcmp("byte", tn)) {
    ...
    if (fixed_len) {
      emit(indent, "%s[bool(x) for x in struct.unpack('>%s%c', buf.read(%d))]%s", accessor, len, _struct_format(lm),
           atoi(len) * _primitive_type_size(tn), suffix);
    } else {
      emit(indent, "%s[bool(x) for x in struct.unpack('>%%d%c' %% self.%s, buf.read(self.%s))]%s", accessor, _struct_format(lm), len, len, suffix);
    }
  } else if (!strcmp("int8_t", tn) || !strcmp("int16_t", tn) || !strcmp("int32_t", tn) || !strcmp("int64_t", tn) || !strcmp("float", tn) ||
             !strcmp("double", tn)) {
    if (fixed_len) {
      emit(indent, "%sstruct.unpack('>%s%c', buf.read(%d))%s", accessor, len, _struct_format(lm), atoi(len) * _primitive_type_size(tn), suffix);
    } else {
      if (_primitive_type_size(tn) > 1) {
        emit(indent, "%sstruct.unpack('>%%d%c' %% self.%s, buf.read(self.%s * %d))%s", accessor, _struct_format(lm), len, len,
             _primitive_type_size(tn), suffix);
      } else {
        emit(indent, "%sstruct.unpack('>%%d%c' %% self.%s, buf.read(self.%s))%s", accessor, _struct_format(lm), len, len, suffix);
      }
    }
  } else {
    assert(0);
...
static void _flush_read_struct_fmt(const ocmgen_t *ocm, FILE *f, GQueue *formats, GQueue *members) {
  int nfmts = g_queue_get_length(formats);
  assert(nfmts == static_cast<int>(g_queue_get_length(members)));
  if (nfmts == 0) return;

  fprintf(f, "        ");
  int fmtsize = 0;
  while (!g_queue_is_empty(members)) {
    ocm_member_t *lm = (ocm_member_t *)g_queue_pop_head(members);
...
static void emit_python_decode_one(const ocmgen_t *ocm, FILE *f, ocm_struct_t *ls) {
  ...
  for (unsigned int m = 0; m < g_ptr_array_size(ls->members); m++) {
    ocm_member_t *lm = (ocm_member_t *)g_ptr_array_index(ls->members, m);
    ...
    if (!lm->dimensions->len) {
      if (fmt && strcmp(lm->type->lctypename, "boolean")) {
        ...
        _flush_read_struct_fmt(ocm, f, struct_fmt, struct_members);
        char *accessor = g_strdup_printf("self.%s = ", lm->membername);
        _emit_decode_one(ocm, f, ls, lm, accessor, 2, "");
        g_free(accessor);
      }
    } else {
      _flush_read_struct_fmt(ocm, f, struct_fmt, struct_members);
      ...
      for (n = 0; n < lm->dimensions->len - 1; n++) {
        ocm_dimension_t *dim = (ocm_dimension_t *)g_ptr_array_index(lm->dimensions, n);
      ...
      // last dimension.
      ocm_dimension_t *last_dim = (ocm_dimension_t *)g_ptr_array_index(lm->dimensions, lm->dimensions->len - 1);
      int last_dim_fixed_len = last_dim->mode == OCM_CONST;

      if (ocm_is_primitive_type(lm->type->lctypename) && 0 != strcmp(lm->type->lctypename, "string")) {
        ...
        _emit_decode_list(ocm, f, ls, lm, accessor->str, 2 + n, n == 0, last_dim->size, last_dim_fixed_len);
      } else {
        // member is either a string type or an inner OCM type.  Each
        ...
        g_string_append_printf(accessor, ".append(");
        _emit_decode_one(ocm, f, ls, lm, accessor->str, n + 3, ")");
      }
...
static void emit_python_decode(const ocmgen_t *ocm, FILE *f, ocm_struct_t *ls) {
  // clang-format off
...
static void _emit_encode_one(const ocmgen_t *ocm, FILE *f, ocm_struct_t *ls, ocm_member_t *lm, const char *accessor, int indent) {
  const char *tn = lm->type->lctypename;
  const char *mn = lm->membername;
  if (!strcmp("string", tn)) {
...
static void _emit_encode_list(const ocmgen_t *ocm, FILE *f, ocm_struct_t *ls, ocm_member_t *lm, const char *accessor, int indent, const char *len,
                              int fixed_len) {
  const char *tn = lm->type->lctypename;
  if (!strcmp("byte", tn)) {
    emit(indent, "buf.write(bytearray(%s[:%s%s]))", accessor, (fixed_len ? "" : "self."), len);
    return;
  } else if (!strcmp("boolean", tn) || !strcmp("int8_t", tn) || !strcmp("int16_t", tn) || !strcmp("int32_t", tn) || !strcmp("int64_t", tn) ||
             !strcmp("float", tn) || !strcmp("double", tn)) {
    if (fixed_len) {
      emit(indent, "buf.write(struct.pack('>%s%c', *%s[:%s]))", len, _struct_format(lm), accessor, len);
    } else {
      emit(indent, "buf.write(struct.pack('>%%d%c' %% self.%s, *%s[:self.%s]))", _struct_format(lm), len, accessor, len);
    }
  } else {
    assert(0);
  }
...
static void emit_python_encode_one(const ocmgen_t *ocm, FILE *f, ocm_struct_t *ls) {
  emit(1, "def _encode_one(self, buf):");
  if (!g_ptr_array_size(ls->members)) emit(2, "pass");
  ...
  for (unsigned int m = 0; m < g_ptr_array_size(ls->members); m++) {
    ...
    if (!lm->dimensions->len) {
      if (fmt) {
        ...
        char *accessor = g_strdup_printf("self.%s", lm->membername);
        _emit_encode_one(ocm, f, ls, lm, accessor, 2);
        g_free(accessor);
      }
    } else {
      _flush_write_struct_fmt(f, struct_fmt, struct_members);
      ...
      if (ocm_is_primitive_type(lm->type->lctypename) && 0 != strcmp(lm->type->lctypename, "string")) {
        _emit_encode_list(ocm, f, ls, lm, accessor->str, 2 + n, last_dim->size, last_dim_fixed_len);
      } else {
        if (last_dim_fixed_len) {
          emit(2 + n, "for i%d in range(%s):", n, last_dim->size);
        } else {
        ...
        g_string_append_printf(accessor, "[i%d]", n);
        _emit_encode_one(ocm, f, ls, lm, accessor->str, n + 3);
      }
...
static void emit_python_encode(const ocmgen_t *ocm, FILE *f, ocm_struct_t *ls) {
...
static void emit_member_initializer(const ocmgen_t *ocm, FILE *f, ocm_member_t *lm, unsigned int dim_num) {
  ...
  ocm_dimension_t *dim = (ocm_dimension_t *)g_ptr_array_index(lm->dimensions, dim_num);
  if (dim->mode == OCM_VAR) {
    fprintf(f, "[]");
  } else {
    fprintf(f, "[ ");
    emit_member_initializer(ocm, f, lm, dim_num + 1);
    fprintf(f, " for dim%d in range(%s) ]", dim_num, dim->size);
  }
}

static void emit_python_init(const ocmgen_t *ocm, FILE *f, ocm_struct_t *lr) {
  fprintf(f, "    def __init__(self):\n");
  unsigned int member;
  for (member = 0; member < lr->members->len; member++) {
    ocm_member_t *lm = (ocm_member_t *)g_ptr_array_index(lr->members, member);
    fprintf(f, "        self.%s = ", lm->membername);

    emit_member_initializer(ocm, f, lm, 0);
    fprintf(f, "\n");
  }
  if (0 == member) {
    fprintf(f, "        pass\n");
...
static void emit_python_fingerprint(const ocmgen_t *ocm, FILE *f, ocm_struct_t *ls) {
  ...
  for (unsigned int m = 0; m < ls->members->len; m++) {
    ocm_member_t *lm = (ocm_member_t *)g_ptr_array_index(ls->members, m);
    if (!ocm_is_primitive_type(lm->type->lctypename)) {
  ...
  // clang-format off
    ...
    emit(2,     "\"\"\"Get the OCM hash of the struct\"\"\"");
    emit(2,     "return struct.unpack(\">Q\", %s._get_packed_fingerprint())[0]", sn);
    fprintf(f, "\n");
}

static void
emit_python_dependencies (const ocmgen_t *ocm, FILE *f, ocm_struct_t *ls)
{
...
static int
emit_package (ocmgen_t *ocm, _package_contents_t *pc)
{
    ...
    int write_init_py = !getopt_get_bool(ocm->gopt, "python-no-init");

    sprintf (package_dir_prefix, "%s%s", getopt_get_string(ocm->gopt, "ppath"), 
            strlen(getopt_get_string(ocm->gopt, "ppath")) > 0 ? 
            G_DIR_SEPARATOR_S : "");
...
//            g_mkdir_with_parents (package_dir, 0755);
    ...
    if (have_package && write_init_py) {
        ...
        for (int i=0 ; i<ndirs; i++) {
            ...
            if(created_initpy) {
                fprintf (init_py_fp, "\"\"\"OCM package __init__.py file\n"
                        "This file automatically generated by ocm-gen.\n"
    ...
    for (int i=0; i<pc->enums->len; i++) {
        ocm_enum_t *le = (ocm_enum_t *) g_ptr_array_index (pc->enums, i);
        ...
        if (!ocm_needs_generation(ocm, le->ocmfile, path))
            continue;

        FILE *f = fopen(path, "w");
        if (f==NULL) return -1;

        fprintf(f, "\"\"\"OCM type definitions\n"
                "This file automatically generated by ocm.\n"
                "DO NOT MODIFY BY HAND!!!!\n"
                "\"\"\"\n"
                "\n"
                "try:\n"
        ...
        for (unsigned int v = 0; v < le->values->len; v++) {
            ocm_enum_value_t *lev = (ocm_enum_value_t *) g_ptr_array_index(le->values, v);
  ...
  for (int i = 0; i < pc->structs->len; i++) {
    ocm_struct_t *ls = (ocm_struct_t *)g_ptr_array_index(pc->structs, i);
    ...
    if (init_py_fp && !g_hash_table_lookup(init_py_imports, ls->structname->shortname))
      fprintf(init_py_fp, "from .%s import %s\n", ls->structname->shortname, ls->structname->shortname);

    if (!ocm_needs_generation(ocm, ls->ocmfile, path)) continue;

    FILE *f = fopen(path, "w");
    if (f == NULL) return -1;

    fprintf(f,
            "\"\"\"OCM type definitions\n"
            "This file automatically generated by ocm.\n"
            "DO NOT MODIFY BY HAND!!!!\n"
            "\"\"\"\n"
            "\n"
    ...
    for (unsigned int cn = 0; cn < g_ptr_array_size(ls->constants); cn++) {
      ocm_constant_t *lc = (ocm_constant_t *)g_ptr_array_index(ls->constants, cn);
      assert(ocm_is_legal_const_type(lc->lctypename));
      emit(1, "%s = %s", lc->membername, lc->val_str);
    }
    if (g_ptr_array_size(ls->constants) > 0) emit(0, "");

    emit_python_init(ocm, f, ls);
    emit_python_encode(ocm, f, ls);
    emit_python_encode_one(ocm, f, ls);
    emit_python_decode(ocm, f, ls);
    emit_python_decode_one(ocm, f, ls);
    emit_python_fingerprint(ocm, f, ls);
    fclose(f);
  }

  if (init_py_fp) fclose(init_py_fp);
  g_hash_table_destroy(init_py_imports);
  return 0;
}

int emit_python(ocmgen_t *ocm) {
  GHashTable *packages = g_hash_table_new_full(g_str_hash, g_str_equal, NULL, (GDestroyNotify)_package_contents_free);

  // group the enums and structs by package
  for (unsigned int i = 0; i < ocm->enums->len; i++) {
    ocm_enum_t *le = (ocm_enum_t *)g_ptr_array_index(ocm->enums, i);
    _package_contents_t *pc = (_package_contents_t *)g_hash_table_lookup(packages, le->enumname->package);
    if (!pc) {
  ...
  for (unsigned int i = 0; i < ocm->structs->len; i++) {
    ocm_struct_t *ls = (ocm_struct_t *)g_ptr_array_index(ocm->structs, i);
    _package_contents_t *pc = (_package_contents_t *)g_hash_table_lookup(packages, ls->structname->package);
    if (!pc) {
  ...
  for (int i = 0; i < vals->len; i++) {
    ...
    int status = emit_package(ocm, pc);
