...
// Some types do not have a 1:1 mapping from ocm types to native C
// storage types. Do not free the string pointers returned by this
// function.
static char *map_type_name(const char *t) {
  if (!strcmp(t, "boolean")) return strdup("int8_t");

  if (!strcmp(t, "string")) return strdup("std::string");

  if (!strcmp(t, "byte")) return strdup("uint8_t");
...
static void emit_package_namespace_start(ocmgen_t *ocmgen, FILE *f, ocm_struct_t *ls) {
  ...
  for (int nsind = 0; namespaces[nsind] && namespaces[nsind + 1]; nsind++) {
    emit(0, "namespace openrobot::ocm::%s\n{", namespaces[nsind]);
  }
  g_strfreev(namespaces);
}

static void emit_package_namespace_close(ocmgen_t *ocmgen, FILE *f, ocm_struct_t *ls) {
  char **namespaces = g_strsplit(ls->structname->lctypename, ".", 0);
  for (int nsind = 0; namespaces[nsind] && namespaces[nsind + 1]; nsind++) {
    emit(0, "}\n");
  }
...
/** Emit header file **/
static void emit_header_start(ocmgen_t *ocmgen, FILE *f, ocm_struct_t *ls) {
  ...
  fprintf(f, "\n");
  fprintf(f, "#include \"ocm_coretypes.h\"\n");
  fprintf(f, "\n");
  ...
  for (unsigned int mind = 0; mind < g_ptr_array_size(ls->members); mind++) {
    ocm_member_t *lm = (ocm_member_t *)g_ptr_array_index(ls->members, mind);
    if (g_ptr_array_size(lm->dimensions) != 0 && !ocm_is_constant_size_array(lm) && !emit_include_vector) {
  ...
  // include header files for other OCM types
  for (unsigned int mind = 0; mind < g_ptr_array_size(ls->members); mind++) {
    ocm_member_t *lm = (ocm_member_t *)g_ptr_array_index(ls->members, mind);

    if (!ocm_is_primitive_type(lm->type->lctypename) && strcmp(lm->type->lctypename, ls->structname->lctypename)) {
      char *other_tn = dots_to_slashes(lm->type->lctypename);
      emit(0, "#include \"%s%s%s.hpp\"", getopt_get_string(ocmgen->gopt, "cpp-include"),
           strlen(getopt_get_string(ocmgen->gopt, "cpp-include")) > 0 ? G_DIR_SEPARATOR_S : "", other_tn);
      free(other_tn);
    }
  }

  fprintf(f, "\n");
  emit_package_namespace_start(ocmgen, f, ls);
  ...
  if (g_ptr_array_size(ls->members)) {
    ...
    for (unsigned int mind = 0; mind < g_ptr_array_size(ls->members); mind++) {
      ...
      if (ndim == 0) {
        ...
        if (ocm_is_constant_size_array(lm)) {
          emit_start(2, "%-10s %s", mapped_typename, lm->membername);
          for (unsigned int d = 0; d < static_cast<unsigned int>(ndim); d++) {
            ocm_dimension_t *ld = (ocm_dimension_t *)g_ptr_array_index(lm->dimensions, d);
            emit_continue("[%s]", ld->size);
          }
          emit_end(";");
        } else {
          emit_start(2, "");
          for (unsigned int d = 0; d < static_cast<unsigned int>(ndim); d++) emit_continue("std::vector< ");
          emit_continue("%s", mapped_typename);
          for (unsigned int d = 0; d < static_cast<unsigned int>(ndim); d++) emit_continue(" >");
  ...
  if (g_ptr_array_size(ls->constants) > 0) {
    ...
    for (unsigned int i = 0; i < g_ptr_array_size(ls->constants); i++) {
      ocm_constant_t *lc = (ocm_constant_t *)g_ptr_array_index(ls->constants, i);
      ...
      if (!strcmp(lc->lctypename, "int32_t")) {
        ...
        const char *suffix = "";
        if (!strcmp(lc->lctypename, "int64_t")) suffix = "LL";
        char *mapped_typename = map_type_name(lc->lctypename);
        char *cpp_std = getopt_get_string(ocmgen->gopt, "cpp-std");
        ...
        if (!strcmp(cpp_std, "c++11")) {
          emit(2, "static constexpr %-8s %s = %s%s;", mapped_typename, lc->membername, lc->val_str, suffix);
        } else {
          ...
          emit(2, "// static data member then re-run ocm-gen with '--cpp-std=c++11'");
          emit(2, "// to generate code that is compliant with C++11");
          emit(2, "static const %-8s %s = %s%s;", mapped_typename, lc->membername, lc->val_str, suffix);
        }
        free(mapped_typename);
      }
    }
  ...
  emit(0, "");
  emit(2, "// OCM support functions. Users should not call these");
  ...
  emit(2, "inline static uint64_t _computeHash(const __ocm_hash_ptr *p);");
  emit(0, "};");
  emit(0, "");
...
static void emit_encode(ocmgen_t *ocm, FILE *f, ocm_struct_t *ls) {
...
static void emit_encoded_size(ocmgen_t *ocm, FILE *f, ocm_struct_t *ls) {
  const char *sn = ls->structname->shortname;
  // clang-format off
...
static void emit_decode(ocmgen_t *ocm, FILE *f, ocm_struct_t *ls) {
  const char *sn = ls->structname->shortname;
  // clang-format off
...
static void emit_get_hash(ocmgen_t *ocm, FILE *f, ocm_struct_t *ls) {
  const char *sn = ls->structname->shortname;
  // clang-format off
...
static void emit_compute_hash(ocmgen_t *ocm, FILE *f, ocm_struct_t *ls)
{
    const char *sn  = ls->structname->shortname;
    ...
    if(last_complex_member >= 0) {
        // clang-format off
        emit(0, "uint64_t %s::_computeHash(const __ocm_hash_ptr *p)", sn);
        emit(0, "{");
        emit(1,     "const __ocm_hash_ptr *fp;");
        emit(1,     "for(fp = p; fp != NULL; fp = fp->parent)");
        emit(2,         "if(fp->v == %s::getHash)", sn);
        emit(3,              "return 0;");
    // clang-format on
    if (g_ptr_array_size(ls->members)) {
      emit(1, "const __ocm_hash_ptr cp = { p, %s::getHash };", sn);
    }
    emit(0, "");
    emit(1, "uint64_t hash = 0x%016" PRIx64 "LL +", ls->hash);

    for (int m = 0; m <= last_complex_member; m++) {
      ocm_member_t *lm = (ocm_member_t *)g_ptr_array_index(ls->members, m);
      char *lm_tnc = dots_to_double_colons(lm->type->lctypename);
      if (!ocm_is_primitive_type(lm->type->lctypename)) {
        emit(2, " %s::_computeHash(&cp)%s", lm_tnc, (m == last_complex_member) ? ";" : " +");
      }
      free(lm_tnc);
    }
    emit(0, "");
    ...
    // clang-format off
        emit(0, "uint64_t %s::_computeHash(const __ocm_hash_ptr *)", sn);
        emit(0, "{");
        emit(1,     "uint64_t hash = 0x%016" PRIx64 "LL;", ls->hash);
    // clang-format on
  }
...
static void _encode_recursive(ocmgen_t *ocm, FILE *f, ocm_member_t *lm, int depth, int extra_indent) {
  int indent = extra_indent + 1 + depth;
  // primitive array
  if (depth + 1 == g_ptr_array_size(lm->dimensions) && ocm_is_primitive_type(lm->type->lctypename) && strcmp(lm->type->lctypename, "string")) {
    ocm_dimension_t *dim = (ocm_dimension_t *)g_ptr_array_index(lm->dimensions, depth);
    emit_start(indent, "tlen = __%s_encode_array(buf, offset + pos, maxlen - pos, &this->%s", lm->type->lctypename, lm->membername);
    for (int i = 0; i < depth; i++) emit_continue("[a%d]", i);
  ...
  emit(indent, "for (int a%d = 0; a%d < %s%s; a%d++) {", depth, depth, dim_size_prefix(dim->size), dim->size, depth);

  _encode_recursive(ocm, f, lm, depth + 1, extra_indent);

  emit(indent, "}");
}

static void emit_encode_nohash(ocmgen_t *ocm, FILE *f, ocm_struct_t *ls) {
  const char *sn = ls->structname->shortname;
    ...
    if (0 == num_dims) {
      if (ocm_is_primitive_type(lm->type->lctypename)) {
        if (!strcmp(lm->type->lctypename, "string")) {
          ...
          emit(1, "tlen = __%s_encode_array(buf, offset + pos, maxlen - pos, &this->%s, 1);", lm->type->lctypename, lm->membername);
        }
        emit(1, "if(tlen < 0) return tlen; else pos += tlen;");
      } else {
        _encode_recursive(ocm, f, lm, 0, 0);
      }
    } else {
      ocm_dimension_t *last_dim = (ocm_dimension_t *)g_ptr_array_index(lm->dimensions, num_dims - 1);
      ...
      if (ocm_is_primitive_type(lm->type->lctypename) && strcmp(lm->type->lctypename, "string") && !is_dim_size_fixed(last_dim->size)) {
        emit(1, "if(%s%s > 0) {", dim_size_prefix(last_dim->size), last_dim->size);
        _encode_recursive(ocm, f, lm, 0, 1);
        emit(1, "}");
      } else {
        _encode_recursive(ocm, f, lm, 0, 0);
      }
...
static void emit_encoded_size_nohash(ocmgen_t *ocm, FILE *f, ocm_struct_t *ls) {
  const char *sn = ls->structname->shortname;
  ...
  for (unsigned int m = 0; m < g_ptr_array_size(ls->members); m++) {
    ...
    if (ocm_is_primitive_type(lm->type->lctypename) && strcmp(lm->type->lctypename, "string")) {
      ...
      if (ndim > 0) {
        ocm_dimension_t *dim = (ocm_dimension_t *)g_ptr_array_index(lm->dimensions, ndim - 1);
        emit_end("__%s_encoded_array_size(NULL, %s%s);", lm->type->lctypename, dim_size_prefix(dim->size), dim->size);
      } else {
      ...
      for (int n = 0; n < ndim; n++) {
        ocm_dimension_t *dim = (ocm_dimension_t *)g_ptr_array_index(lm->dimensions, n);
        emit(1 + n, "for (int a%d = 0; a%d < %s%s; a%d++) {", n, n, dim_size_prefix(dim->size), dim->size, n);
      }
      emit_start(ndim + 1, "enc_size += this->%s", lm->membername);
      for (int i = 0; i < ndim; i++) emit_continue("[a%d]", i);
      if (!strcmp(lm->type->lctypename, "string")) {
        emit_end(".size() + 4 + 1;");
      } else {
...
static void _decode_recursive(ocmgen_t *ocm, FILE *f, ocm_member_t *lm, int depth) {
  // primitive array
  if (depth + 1 == g_ptr_array_size(lm->dimensions) && ocm_is_primitive_type(lm->type->lctypename) && strcmp(lm->type->lctypename, "string")) {
    ocm_dimension_t *dim = (ocm_dimension_t *)g_ptr_array_index(lm->dimensions, depth);
    ...
    emit_start(decode_indent, "tlen = __%s_decode_array(buf, offset + pos, maxlen - pos, &this->%s", lm->type->lctypename, lm->membername);
    for (int i = 0; i < depth; i++) emit_continue("[a%d]", i);
    emit_end("[0], %s%s);", dim_size_prefix(dim->size), dim->size);
    ...
    emit(1 + depth, "for (int a%d = 0; a%d < %s%s; a%d++) {", depth, depth, dim_size_prefix(dim->size), dim->size, depth);

    _decode_recursive(ocm, f, lm, depth + 1);

    emit(1 + depth, "}");
  }
}

static void emit_decode_nohash(ocmgen_t *ocm, FILE *f, ocm_struct_t *ls) {
  const char *sn = ls->structname->shortname;
  if (0 == g_ptr_array_size(ls->members)) {
    // clang-format off
  ...
  for (unsigned int m = 0; m < g_ptr_array_size(ls->members); m++) {
    ...
    if (0 == g_ptr_array_size(lm->dimensions) && ocm_is_primitive_type(lm->type->lctypename)) {
      if (!strcmp(lm->type->lctypename, "string")) {
        ...
        emit(1, "    static_cast<const char*>(buf) + offset + pos, __%s_len__ - 1);", lm->membername);
...
int emit_cpp(ocmgen_t *ocmgen) {
  ...
  for (unsigned int i = 0; i < g_ptr_array_size(ocmgen->structs); i++) {
    ...
    char *header_name = g_strdup_printf("%s%s%s.hpp", getopt_get_string(ocmgen->gopt, "cpp-hpath"),
                                        strlen(getopt_get_string(ocmgen->gopt, "cpp-hpath")) > 0 ? G_DIR_SEPARATOR_S : "", tn_);

    // generate code if needed
    if (ocm_needs_generation(ocmgen, lr->ocmfile, header_name)) {
      make_dirs_for_file(header_name);

      FILE *f = fopen(header_name, "w");
      if (f == NULL) return -1;

      emit_header_start(ocmgen, f, lr);
      emit_encode(ocmgen, f, lr);
      emit_decode(ocmgen, f, lr);
      emit_encoded_size(ocmgen, f, lr);
      emit_get_hash(ocmgen, f, lr);
      ...
      emit_encode_nohash(ocmgen, f, lr);
      emit_decode_nohash(ocmgen, f, lr);
      emit_encoded_size_nohash(ocmgen, f, lr);
      emit_compute_hash(ocmgen, f, lr);

      emit_package_namespace_close(ocmgen, f, lr);
