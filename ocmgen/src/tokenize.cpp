...
int tokenize_next_char(tokenize_t *t) {
  ...
  if (t->buffer_column == t->buffer_len) {
    ...
    if (fgets(t->buffer, MAX_LINE_LEN, t->f) == NULL) return EOF;
...
int tokenize_next_internal(tokenize_t *t) {
  ...
  if (c == '\'') {
    t->token[pos++] = c;
    c = tokenize_next_char(t);
    if (c == '\\') c = unescape(tokenize_next_char(t));
    if (c == EOF) return -4;
    t->token[pos++] = c;
    c = tokenize_next_char(t);
    if (c != '\'') return -5;
    t->token[pos++] = c;
    t->token_type = OCM_TOK_OTHER;
    goto end_tok;
  }
  ...
  do {
    ...
    if (strchr(single_char_toks, c) != NULL) goto end_tok;
...
int tokenize_peek(tokenize_t *t) {
  ...
  if (t->hasnext) return res;

  res = tokenize_next(t);
  if (res != EOF) t->hasnext = 1;
